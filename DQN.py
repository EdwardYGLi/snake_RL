...
class DQNCNN(nn.Module):
    def __init__(self, side_length, outputs, in_channels = 1, features=[32, 64, 128]):
        ...
        self.conv1 = nn.Conv2d(in_channels=in_channels, out_channels=features[0], kernel_size=5, stride=2, padding=2)
        self.bn1 = nn.BatchNorm2d(num_features=features[0])
        # self.mp1 = nn.MaxPool2d(kernel_size=2, stride=2)
        self.conv2 = nn.Conv2d(in_channels=features[0], out_channels=features[1], kernel_size=5, stride=2, padding=2)
        self.bn2 = nn.BatchNorm2d(num_features=features[1])
        # self.mp2 = nn.MaxPool2d(kernel_size=2, stride=2)
        self.conv3 = nn.Conv2d(in_channels=features[1], out_channels=features[2], kernel_size=5, stride=1, padding=2)
        ...
        # calculate shape
        side_length = calc_out_shape(side_length, 5, 2, 2)  # after first conv
        # side_length = calc_out_shape(side_length, 2, 2, 0)  # after first maxpool
        side_length = calc_out_shape(side_length, 5, 2, 2)  # after second conv
        # side_length = calc_out_shape(side_length, 2, 2, 0)  # after second maxpool
        side_length = calc_out_shape(side_length, 5, 1, 2)  # after third conv
